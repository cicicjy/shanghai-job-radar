...
function isReadableDescription(text = "") {
  ...
  if (/href|class|system-path|main-menu|node\/\d+|data-[a-z-]+/i.test(value)) return false;
...
function isSpecificJobUrl(url, source) {
  ...
  if (/\/careers\/teams?\/|\/teams?\//i.test(url)) return false;
...
function extractJsonLdJobs(html, source, pageUrl) {
  ...
  for (const match of scripts) {
    ...
    for (const posting of flattenJobPostings(json)) {
      ...
      if (!title || !hasTargetKeyword(text) || !hasShanghaiSignal(text, pageUrl, source) || hasNegativeTitle(title) || isGenericLandingTitle(title)) {
        continue;
      }
      jobs.push({
        title,
...
function extractAnchorJobs(html, source, pageUrl) {
  ...
  for (const match of anchors) {
    ...
    if (!title || title.length < 4 || title.length > 140 || hasNegativeTitle(title) || isGenericLandingTitle(title)) continue;
...
function extractEmbeddedJsonJobs(html, source, pageUrl) {
  ...
  for (const match of titleMatches.slice(0, 220)) {
    ...
    if (!title || hasNegativeTitle(title) || isGenericLandingTitle(title)) continue;
    const snippet = cleanJobText(html.slice(Math.max(0, match.index - 1200), Math.min(html.length, match.index + 2600)));
