...
function sourceCompanyText(source = {}) {
  return [
    source.id,
    source.company,
...
function sourceMatchesCompany(source, query) {
  const rawQuery = cleanText(query);
  if (!rawQuery) return true;
  const haystack = normalizeSearchText(sourceCompanyText(source));
  const compactHaystack = compactSearchText(sourceCompanyText(source));
  const normalizedQuery = normalizeSearchText(rawQuery);
  const compactQuery = compactSearchText(rawQuery);
  if (!normalizedQuery) return true;
  if (haystack.includes(normalizedQuery) || compactHaystack.includes(compactQuery)) return true;
...
function sourceMatchesJobCompany(job, query) {
  const source = careerSources.find((item) => item.id === job.sourceId);
  if (source) return sourceMatchesCompany(source, query);
  const haystack = normalizeSearchText(`${job.company || ""} ${(job.brands || []).join(" ")}`);
  const compactHaystack = compactSearchText(`${job.company || ""} ${(job.brands || []).join(" ")}`);
  const normalizedQuery = normalizeSearchText(query);
  const compactQuery = compactSearchText(query);
  return Boolean(normalizedQuery && (haystack.includes(normalizedQuery) || compactHaystack.includes(compactQuery)));
}
...
function looksLikeJobRecord(value) {
  ...
  const title = value.title || value.jobTitle || value.name || value.positionTitle || value.postName || value.positionName;
...
function collectJobRecords(value, result = [], seen = new Set()) {
  ...
  if (looksLikeJobRecord(value)) {
    const key =
      ...
      value.externalPostId ||
      value.postId ||
      `${value.title || value.postName}|${value.location || value.city || value.workPlaceStr || ""}`;
    if (!seen.has(key)) {
      seen.add(key);
      result.push(value);
    }
...
function extractCodeJsonJobs(html, source, pageUrl) {
  const jobs = [];
  const blocks = html.matchAll(/<code[^>]+id=["'][^"']*(?:data|pcsx|initial|state)[^"']*["'][^>]*>([\s\S]*?)<\/code>/gi);
  for (const match of blocks) {
    const json = parseMaybeJson(cleanText(match[1]));
    if (json) jobs.push(...extractApiJobs(json, source, pageUrl).map((job) => ({ ...job, sourceType: "official-embedded" })));
  }
  return jobs;
}
...
function normalizeApiJobUrl(data, source) {
  ...
  if (data.postId && (data.currentSuiteKey || data.suiteKey || source.id === "decathlon")) {
    const suiteKey = data.currentSuiteKey || data.suiteKey || "64631fe6bef57c0907f133c4";
    const projectCode = data.projectCode || data.projectId || "1002101";
    return `https://wecruit.hotjob.cn/SU${suiteKey}/pb/posDetail.html?postId=${encodeURIComponent(data.postId)}&projectCode=${encodeURIComponent(projectCode)}&showProjectBanner=true`;
  }
...
function extractApiJobs(payload, source, pageUrl) {
  ...
  for (const record of records) {
    ...
    const title = compactCleanText(data.title || data.jobTitle || data.name || data.postName || data.positionName || "");
    if (isInvalidJobTitle(title)) continue;
    const categoryText = [
      data.category,
      data.categories,
    ...
    const jobType = data.type || data.employmentType || data.workerType || data.workType || data.workTypeStr || data.schedule;
    ...
    jobs.push({
      ...
      description,
      datePosted: cleanText(data.posted_date || data.datePosted || data.postedDate || data.publishDate || data.publishFirstDate || data.create_date || data.dateCreated || data.update_date || ""),
      jobId: cleanText(data.req_id || data.reqId || data.jobId || data.job_id || data.jobSeqNo || data.requisitionId || data.postCode || data.externalPostId || data.postId || data.id || data.slug || extractJobId(text, url)),
...
function isBlockedHtml(html = "", status = 200) {
  const value = String(html || "").slice(0, 20000);
  return (
    status === 403 ||
    /Just a moment|cf-chl|cf_chl|Cloudflare|Enable JavaScript and cookies to continue|DataDome|AkamaiGHost|Access Denied|Bot Management/i.test(value)
  );
}
...
function sourceMeta(source) {
  ...
  if (source.leadSources) meta.leadSources = source.leadSources;
  return meta;
}
  ...
  for (const url of urls) {
    try {
      ...
      if (!fetched.ok) {
        ...
        urlResults.push({ url, finalUrl: pageUrl, ok: false, status: fetched.status, extractedCount: 0 });
        continue;
      }
      const html = fetched.text;
      if (isBlockedHtml(html, fetched.status)) {
        errors.push(`受限/需浏览器 ${url}`);
        urlResults.push({ url, finalUrl: pageUrl, ok: false, status: fetched.status, blocked: true, extractedCount: 0 });
        continue;
      }
      if (/json/i.test(contentType) || /^[\s\n\r]*[{[]/.test(html)) {
        ...
        jobs.push(...extractCodeJsonJobs(html, source, pageUrl));
      ...
      urlResults.push({
        ...
        contentType,
        extractedCount: jobs.length - beforeCount,
      });
    } catch (error) {
      errors.push(`${errorLabel(error)} ${url}`);
      urlResults.push({ url, finalUrl: url, ok: false, status: 0, extractedCount: 0, error: errorLabel(error) });
    }
  }

  const fullTimeCandidates = jobs.filter((job) => isFullTimeJob(job));
  const normalizedCandidates = unique(
    fullTimeCandidates.map((job) => normalizeJob(job, source)),
    (job) => (job.jobId ? `${job.company}|${job.jobId}` : job.url && job.url !== source.careersUrl ? job.url : `${job.company}|${job.title}|${job.location}`),
  );
  const validCandidates = normalizedCandidates.filter((job) => !isInvalidJobTitle(job.title) && isFullTimeJob(job));
  const shanghaiCandidates = validCandidates.filter((job) => isShanghaiJob(`${job.title} ${job.location}`, job.url));
  const targetCandidates = shanghaiCandidates.filter((job) => hasTargetRoleSignal(job) && !isNonTargetRole(job));
  const normalized = targetCandidates
    .filter((job) => job.match.score >= (options.minScore || 35))
    .sort((a, b) => b.match.score - a.match.score);

  const audit = {
    checkedAt: new Date().toISOString(),
    capturedJobCount: normalizedCandidates.length,
    fullTimeJobCount: fullTimeCandidates.length,
    shanghaiJobCount: shanghaiCandidates.length,
    targetRoleCount: targetCandidates.length,
    matchGt50Count: targetCandidates.filter((job) => job.match.score >= 50).length,
    finalVisibleJobCount: normalized.length,
    topScore: targetCandidates.reduce((max, job) => Math.max(max, job.match?.score || 0), 0),
    scannedUrlCount: urls.length,
    successfulUrlCount: urlResults.filter((item) => item.ok).length,
    blockedUrlCount: urlResults.filter((item) => item.blocked).length,
    failedUrlCount: urlResults.filter((item) => !item.ok).length,
    status:
      urlResults.some((item) => item.blocked)
        ? "blocked"
        : normalizedCandidates.length
          ? shanghaiCandidates.length
          ...
          : errors.length
            ? "fetch_error"
            : "no_jobs_captured",
    urlResults: urlResults.slice(0, 8),
  };
...
function filterSources({ sourceLimit, priority, company, sourceId, originRegion, industry } = {}) {
  ...
  if (company) {
    sources = sources.filter((source) => sourceMatchesCompany(source, company));
    return sources.slice(0, sourceLimit || DEFAULT_SOURCE_LIMIT);
  }
...
function filterJobs(jobs, filters = {}) {
  ...
  if (filters.sourceId) {
    result = result.filter((job) => job.sourceId === filters.sourceId);
  } else if (filters.company) {
    result = result.filter((job) => sourceMatchesJobCompany(job, filters.company));
  }
  if (filters.query) {
...
async function fetchJobs(options = {}) {
  ...
  const supplementalSources = unique(
    sources.flatMap((source) => source.leadSources || []),
    (item) => `${item.label}|${item.url}`,
  ).slice(0, 12);
  const auditCandidates = sourceResults.filter((result) => result.audit);
  const randomSample = auditCandidates.length
    ? auditCandidates[Math.floor(Math.random() * auditCandidates.length)]
    : null;
  return {
    jobs: filteredJobs,
    meta: {
      ...
      supplementalSources,
      sourceAudit: randomSample
        ? {
            threshold: 50,
            randomSample: {
              id: randomSample.sourceId,
              company: randomSample.company,
              careersUrl: randomSample.careersUrl,
              status: randomSample.audit.status,
              capturedJobCount: randomSample.audit.capturedJobCount,
              shanghaiJobCount: randomSample.audit.shanghaiJobCount,
              matchGtThresholdCount: randomSample.audit.matchGt50Count,
              finalVisibleJobCount: randomSample.audit.finalVisibleJobCount,
              topScore: randomSample.audit.topScore,
              errors: randomSample.errors.slice(0, 2),
              checkedAt: randomSample.audit.checkedAt,
            },
          }
        : null,
